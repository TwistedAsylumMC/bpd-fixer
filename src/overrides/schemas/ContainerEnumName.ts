...
/**
 * Referenced with `Enum-as-Value` from `FullContainerName./Container Name` and
 * `LegacySetSlot./Container Enum`, so the ordinal is the wire value. No members
 * are missing, but upstream lists them in declaration order rather than value
 * order: `RecipeFoodContainer`/`RecipeBlocksContainer`/`RecipeFurnaceItemsContainer`
 * are 64/65/66 yet appear at positions 18/19/20, shifting 49 members down by three.
 *
...
const override: Override = {
  reason:
    'Members are listed in declaration order; the three Recipe*Container values 64-66 sit at positions 18-20 and shift 49 members. Reordered by value.',
  expect: expectEnumMembers(members, 67),
  root: { enum: members },
};
